...
class Matching ():
	...
	def test (cls, *args, **kwargs):
		return cls('20210909QR_MultiCpu_010.mp4', 21, 99, aruco_model = ArUcoModel.withTagSet('DICT_4X4_100'), *args, **kwargs)
	...
	def match (self):
		...
		def framesPerThread ():
			...
			rolling_window_start = self.video_first_frame + self.half_rolling_window_size
			rolling_window_end = self.video_last_frame - self.half_rolling_window_size + 1
			
			# Assign the frames per thread
			frames_per_thread = math.ceil((rolling_window_end - rolling_window_start) / self.threads)

			# Return windows of equal size for each thread
			for frame in range(rolling_window_start, rolling_window_end, frames_per_thread):
				frame_start = max([self.video_first_frame, frame - self.half_rolling_window_size])
				frame_end = min([self.video_last_frame, frame + frames_per_thread + self.half_rolling_window_size - 1])
	...
	def _matchJob (self, frame_start:int, frame_end:int):
		...
		def croppedCoords (coord, coord_max):
			return np.maximum(int(coord) - self.aruco_crop_size, 0), np.minimum(int(coord) + self.aruco_crop_size, coord_max - 1)
		...
		while frame and current_frame < frame_end:
			...
			frame.cropArUcoWithCoordsArray(test_array, self.aruco_crop_size)
		...
		job_cost_matrix = CostMatrix.fromDict(job_match_dict, frame_start, frame_end, self.half_rolling_window_size)
...
class MatchFrame ():
	...
	def cropArUcoWithCoordsArray (self, coords_array:np.array, crop_size:int):
		...
		def croppedCoords (coord, crop_size, coord_max):
			return np.maximum(int(coord) - crop_size, 0), np.minimum(int(coord) + crop_size, coord_max - 1)
