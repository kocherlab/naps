...
class ArUcoModel ():
	def __init__ (self,
		...
		self.aruco_params.perspectiveRemoveIgnoredMarginPerCell = perspectiveRemoveIgnoredMarginPerCell
	...
	def _assignArucoDict (self, tag_set):
		...
		if not tag_set: raise Exception('No tag set defined')
		if tag_set not in ARUCO_DICT: raise Exception(f'Unable to assign tag set: {tag_set}')
