...
@pytest.mark.parametrize('tag_set', ['DICT_4X4_50',
									 'DICT_4X4_100',
									 'DICT_4X4_250',
									 'DICT_4X4_1000',
									 'DICT_5X5_50',
...
def test_ArUcoModel_tag_sets (tag_set):
	...
	try: ArUcoModel.withTagSet(tag_set)
	except Exception as exc:
		assert False, f'Tag set {tag_set} raised an exception {exc}'

@pytest.mark.parametrize('param, value', [('adaptiveThreshWinSizeMin', 10),
...
def test_ArUcoModel_params (param, value):
	...
	try: ArUcoModel.withTagSet('DICT_4X4_100', **{param: value})
	except Exception as exc:
		assert False, f'Parameter{param} with value {value} raised an exception {exc}'


@pytest.mark.parametrize('param, value', [('adaptiveThreshWinSizeMin', 0.1),
...
def test_ArUcoModel_params_type_error (param, value):
	with pytest.raises(Exception) as e_info: ArUcoModel.withTagSet('DICT_4X4_100', **{param: value})
