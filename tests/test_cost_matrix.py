...
def test_CostMatrix__linearAssignment ():

	# Create a dataframe of basic counts from a dict
	value_dict = {'a':{1:4, 2:2, 3:3}, 'b':{1:1, 2:0, 3:2}, 'c':{1:3, 2:5, 3:2}}
	value_array = pd.DataFrame.from_dict(value_dict).fillna(0).values

	# Store the linear assignment track order
	track_indices = [track_index for track_index, _ in CostMatrix._linearAssignment(value_array)]
